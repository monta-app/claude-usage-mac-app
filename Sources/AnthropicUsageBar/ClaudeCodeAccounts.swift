...
enum CCLogin {
    ...
    /// Snapshot the CURRENT Keychain login into this account's own config-dir
    /// file, WITHOUT re-logging-in. Runs in Terminal (so its Keychain read
    /// persists via Terminal's own Always-Allow). Resets the dir to a clean
    /// file-only state so `claude` reads the file, not the Keychain.
    static func captureCurrent(configDir: String) {
        let script = """
        #!/bin/bash
        DIR="\(configDir)"
        clear
        echo "Saving your CURRENT Claude login as this account…"
        ...
        if [ -z "$CRED" ]; then
          ...
          echo "✅ Saved. This account is now independent of your default login."
        fi
        echo
